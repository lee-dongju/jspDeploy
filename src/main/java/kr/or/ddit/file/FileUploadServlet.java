...
//		logger.debug("uploadFile : " + request.getParameter("uploadFile"));
		...
		//form-data; name="uploardFile; fileName="sally png");
		...
		//파일 저장 경로 설정
		//실무 : 별도 storage
		//개발환경 : 임의 영역(서버 변경시, 재기동시 업로드 파일 삭제 됨)
		// /fileUpload 디렉토리에 임의로 작성
		
		// url -> realpath : application 객체에서 제공해 주는 매서드
		// doPost 매서드에서 application(servletContext)객체를 구한다.
		
		//1. request.getServletContext()
		//2. getServletContext()(서블릿에서 제공해주는 매서드)
